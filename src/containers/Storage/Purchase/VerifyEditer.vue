  ...
  data () {
    return {
      ...
      toolbarList: [{
        ...
        label: '保存材料计划', name: 'save', icon: 'save', style: 'primary'
      }],
      operationList: [{
        label: '编辑', name: 'update', icon: 'edit'
      }],
      tableList: ['productId', 'type', 'size', 'unit', 'num', 'deliveryTime', 'expiryTime', 'shelfLife', 'supplier', 'warehouse', 'source', 'state'],
      updateList: ['deliveryTime', 'expiryTime', 'shelfLife', 'supplier', 'warehouse', 'source', 'state'],
      columns: {
        productId: {
          label: '材料',
          type: 'select',
  ...
  methods: {
    getRelation (value) {
      ...
      .then((data) => {
        ...
        console.log(this.columns.productId.options)
        this.columns.type.options = data.type
        this.columns.unit.options = data.unit
        this.columns.size.options = data.size
        this.columns.state.options = data.state
        this.columns.source.options = data.source
        this.columns.warehouse.options = data.warehouse
        this.getData()
      })
    ...
    toolbarSave () {
      const data = this.tableData.map(item => {
        const { deliveryTime, expiryTime, shelfLife, supplier, warehouse, source } = item
        return { deliveryTime, expiryTime, shelfLife, supplier, warehouse, source, id: item.id }
      })
      fetchUpdate({
        id: this.flowId,
        resource: 'productInOrderDetail',
        data: data
      }).then((data) => {
        if (data) {
          this.$router.push('/base/task/' + this.flowId)
