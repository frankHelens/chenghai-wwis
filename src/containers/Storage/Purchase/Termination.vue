...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/purchase/termination')
    return {
      flowDefineId: menu.flowid,
      resource: 'productInOrder',
      title: '采购终止',
      label: '采购终止',
      labelName: 'name',
      relation: true,
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '采购终止').id
        }]
