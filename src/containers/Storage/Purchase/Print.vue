...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/purchase/print')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '打印入仓单').id
        }]
