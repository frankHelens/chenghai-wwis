...
export default {
  ...
  data () {
    return {
      ...
      tableList: ['productId', 'type', 'size', 'unit', 'num'],
      createList: ['productId', 'num'],
      updateList: ['productId', 'num'],
      columns: {
        productId: {
          label: '材料',
          type: 'select',
  ...
  methods: {
    getRelation (value) {
      fetchList({
        resource: 'productDemandOrderDetail/relation'
      })
      .then((data) => {
    ...
    getData () {
      ...
      this.onLoad = true
      fetchList({
        resource: 'productDemandOrderDetail/' + this.flowId
      })
      .then((data) => {
        this.onLoad = false
    ...
    toolbarSave () {
      const data = this.tableData.map(item => {
        const { productId, num } = item
        return { productId, num }
      })
      fetchUpdate({
        resource: 'productDemandOrderDetail',
        id: this.flowId,
        data: data
      }).then((data) => {
        if (data) {
