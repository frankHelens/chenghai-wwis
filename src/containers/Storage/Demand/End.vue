...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/demand/end')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '物资需求结束').id
        }]
