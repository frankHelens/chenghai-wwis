...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/demand/termination')
    return {
      flowDefineId: menu.flowid,
      resource: 'productDemandOrder',
      title: '物资需求完成',
      label: '物资需求完成',
      labelName: 'name',
      relation: true,
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '物资需求确认').id
        }, {
          name: 'orderStatus',
          type: 'equalto',
          value: 2
