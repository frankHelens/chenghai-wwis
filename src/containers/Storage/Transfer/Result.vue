...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/transfer/result')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '器材股验收').id
        }]
