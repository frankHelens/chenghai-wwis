...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/transfer/fill')
    return {
      ...
      tableInitList: ['orderCode', 'applyDeptName', 'applyUserName'],
      tableFullList: ['orderCode', 'applyDeptName', 'applyUserName'],
      columns: {
        type: {
          label: '需求类型',
          type: 'select'
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '预借转领用信息填写').id
        }]
