...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/storage/receive/end')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '物资出仓结束').id
        }]
