...
export default {
  ...
  data () {
    return {
      ...
      tableList: ['productId', 'type', 'size', 'unit', 'useNum', 'source'],
      columns: {
        productId: {
          label: '材料',
          type: 'select'
  ...
  methods: {
    getRelation (value) {
      fetchList({
        resource: 'productOutOrderDetail/relation'
      })
      .then((data) => {
    ...
    getData () {
      ...
      this.onLoad = true
      fetchList({
        resource: 'productOutOrderDetail/' + this.flowId
      })
      .then((data) => {
        this.onLoad = false
    ...
    toolbarSave () {
      const data = this.tableData.map(item => {
        const { backNum } = item
        return { backNum, id: item.id }
      })
      fetchUpdate({
        resource: 'productOutOrderDetail',
        id: this.flowId,
        data: data
      }).then((data) => {
        this.$router.push('/base/task/' + this.flowId)
