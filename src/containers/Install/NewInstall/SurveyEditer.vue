...
export default {
  ...
  data () {
    return {
      ...
      dialogSize: 'tiny',
      toolbarList: [{
        label: '新增材料', name: 'create', icon: 'plus'
      }, {
      ...
      tableList: ['productId', 'type', 'unit', 'planNum'],
  ...
  computed: {
    ...
    tableColumns () {
      return this.tableList.map(key => this.columns[key])
    }, /* 计算table显示的字段 */
    createColumns () {
      return this.createList.map(key => this.columns[key])
    }, /* 计算新增表单的字段 */
    updateColumns () {
      return this.updateList.map(key => this.columns[key])
    }, /* 计算编辑表单的字段 */
    tableStatus () {
  ...
  methods: {
    getRelation (value) {
      ...
      .then((data) => {
        ...
        this.columns.type.options = data.productId.map(item => {
          return {
            value: item.id,
            label: item.type
          }
        })
        this.columns.unit.options = data.productId.map(item => {
          return {
            value: item.id,
            label: item.unit
          }
    ...
    createFormEvent (name, values, columns) {
      if (name === 'submit') {
        this.tableData.push(cloneDeep(values))
