...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/newInstall/installUnite')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '安装联合勘查').id
        }]
