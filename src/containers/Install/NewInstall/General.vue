...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/newInstall/general')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '总经理审批预算').id
        }]
