...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/newInstall/verify')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '审核工程预算').id
        }]
