...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/newInstall/business')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '营业联合勘查').id
        }]
