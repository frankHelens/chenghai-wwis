...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/newInstall/charge')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '确认收费信息').id
        }]
