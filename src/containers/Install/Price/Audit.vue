...
export default {
  ...
  data () {
    const menu = getMenus().find(item => item.menuurl === '/install/price/audit')
    return {
    ...
    getRelation () {
      ...
      .then((data) => {
        ...
        this.filterByInit = [{
          name: 'stepDefineId',
          type: 'equalto',
          value: data.stepDefineId.find(item => item.flowDefineId === this.flowDefineId && item.label === '审核调价').id
        }]
